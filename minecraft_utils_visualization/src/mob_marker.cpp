...
: rclcpp::Node("mob_marker", options)
{
    ...
    mob_sub_ = this->create_subscription<minecraft_msgs::msg::LivingEntityArray>(
        "player/nearby_living_entities", qos, std::bind(&MobMarker::mobCallback, this, std::placeholders::_1));
}
...
void MobMarker::mobCallback(const minecraft_msgs::msg::LivingEntityArray::SharedPtr msg)
{
    ...
    RCLCPP_DEBUG(this->get_logger(), "Player position: x=%f, y=%f, z=%f", 
    ...
    for (size_t i = 0; i < msg->entities.size(); ++i) {
        ...
        RCLCPP_DEBUG(this->get_logger(), "Mob %d position (player relative): x=%f, y=%f, z=%f", 
                    entity.id, marker.pose.position.x, marker.pose.position.y, marker.pose.position.z);
        
        marker_array.markers.push_back(marker);
        
        this->addNameHealthMarker(entity, marker.header, player_x, player_y, player_z, player_orientation, marker_array);
    }
